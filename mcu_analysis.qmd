...
df <- df %>%
  mutate(
    ...
    sex = str_to_lower(sex) %>% recode("wanita"="Female","perempuan"="Female","laki-laki"="Male","pria"="Male", .default = sex)
  )
...
# Population Insight (prompt)

```{r}
cat('summary analysis of mcu result, count demographic by sex and age:')
z = df %>% rename(age = usia) %>% group_by(sex, age) %>% summarize(count = n()) 
...
```{r}
cat('summary analysis of mcu result, bmi distribution:')
z <- df %>%
  count(parameter_bmi, name = "count") %>%
  arrange(desc(count))
print(as.data.frame(z))
...
```{r}
cat('summary analysis of mcu result, bmi distribution breakdown by age and sex')
z <- df %>% group_by(age_group, sex, parameter_bmi) %>%
  summarize(count = n()) %>%
  arrange(desc(count))
print(as.data.frame(z))
...
# Measured Diabetes (prompt)

```{r}
cat('summary analysis of mcu result blood sugar level breakdown by sex and age group:')
z <- gd_count %>% rename(diabetes_status = gd_label, count=n, proportion = p) %>% select(-lbl)
print(as.data.frame(z))
```
...
# Measured Diabetes: Pre-Diabetes (prompt)

```{r}
cat('summary analysis of mcu result on prediabetic participant only breakdown by sex and age group:')
z <- kuning_summary %>% rename(count = kuning_n , proportion=prop_kuning) %>% select(-prop_signed)
print(as.data.frame(z))
```
...
# Possible Chronic Kidney Disease (prompt)

```{r}
cat('summary analysis of mcu result on Possible Chronic Kidney Disease:')
z <- ginjal_count %>% rename(count = n , proportion=p) %>% select(-label)
print(as.data.frame(z))
```
...
# Possible Chronic Kidney Disease: Focus on Mild-Moderate issues (prompt)

```{r}
cat('summary analysis of mcu result on Possible Chronic Kidney Disease, Focus on Mild-Moderate issues breakdown by sex and age group:')
z <- kuning_summary %>% rename(count = kuning_n  , proportion=prop_kuning ) %>% select(-prop_signed)
print(as.data.frame(z))
```
...
# Cholesterol Paramater (prompt)

```{r}
cat('summary analysis of mcu result on Cholesterol Paramater:')
z <- ginjal_count %>% rename(count = n  , proportion=p ) %>% select(-label)
print(as.data.frame(z))
```

# Cholesterol Paramater breakdown by Age and sex
...
# Cholesterol Paramater breakdown by Age and sex (prompt)

```{r}
cat('summary analysis of mcu result on Cholesterol Paramater breakdown by age and sex:')
z <- df_summary %>% rename(cholesterol_level=parameter_kolesterol, count = n  , proportion=pct ) 
print(as.data.frame(z))
```
...
-   Specials things to note:
    ...
    -   Encouraging participants for regular hydration is key on maintaining uric acid levels -There maybe some participants needing more care (e.g. those developed gout / kidney stones) and may need medication to control uric acid levels

# Uric Acid (prompt)

```{r}
cat('summary analysis of mcu result on distribution of uric acid level:')
z <- asam_summary %>% rename(uric_acid=asam, count = n  , proportion=pct )  %>% select(-label)
print(as.data.frame(z))
```
...
```{r}
cat('summary analysis of mcu result on distribution of uric acid level focus on Hyperuricemia breakdown by sex and age:')
z <- df_pct %>% rename(count=hyper   ,  proportion=pct )  %>% select(-pct_signed)
print(as.data.frame(z))
```
...
The data shows clear age-related risk accumulation, with metabolic and cardiovascular issues clustering over time. Preventive interventions should start before age 30, targeting both sexes, with stronger emphasis on early male screening and sustained female monitoring after 35.

# Multiple Risk Factors (prompt)
...
summary analysis of mcu result on participants with more then 3 risk factor breakdown by gender and age:
    
    ')
z <- risk_summary %>% rename(count=n_multi_risk    ,  proportion=pct_multi_risk ) 
print(as.data.frame(z))
...
diagnosis_freq <- df %>%
  select(diagnose) %>% rename(diagnoses = diagnose) %>%
  filter(!is.na(diagnoses) & diagnoses != "") %>%
  mutate(diagnoses =  str_replace_all(diagnoses, '; ', ';')) %>%
  mutate(diagnoses =  str_replace_all(diagnoses, 'Berdasarkan pemeriksaan yang telah dilakukan dapat disimpulkan sebagai berikut :', '')) %>%
...
# Top 15 Most Frequent Diagnosis (prompt)

```{r}
cat('summary analysis of mcu result on top 15 diagnosis')
z <- diagnosis_freq %>%
  slice_max(count, n = 15)
print(as.data.frame(z))
